return {
    ...
    ANNOUNCE_CAFFINE_BUFF_START = "훌륭해, 시간을 쫒을수 있을것 같아!" , -- 카페인 버프 시작
	...
	DESCRIBE =
	{
		...
		WELL_SPRINKLER_KIT = "정교하면서 설치하는데 1초도 안 걸릴거야.", --스프링클러 키트
		...
		ADDITIVE_PETALS = "오랜 아름다움을 갈망하는 늙은 꽃잎사귀 뭉치야.", -- 리큐르에 제조에 첨가 할 허브,녹차,꽃잎 첨가제
		...
		BUTTERBEER = "밥 먹을 시간을 아낄수 있겠어.", --해리포터 시리즈의 버터비어 

		-- 콜라보 주전자 음료

		COCONUT_JUICE = "속이 비어있는 부분만큼 젤리로 바꿔 보상했어.", --난파선 코코넛 쥬스
		RAINBOW_SHAKE = "효율적이야!", --난파선 무지개 해파리 쉐이크
		QUARTZ_SHAKE = "적어도 생선 씹을 시간은 아꼈네.", --난파선 광대 물고기 쉐이크
		CHERRY_ROSEBERRIES_TEA = "가끔은 꽃 보면서 차 한잔 하는것도 좋을지도 모르지..", --체리숲 로즈힙 차
		CHERRY_HONEY_TEA = "고급스러우면서 따뜻해.", --체리숲 벚꽃꿀 차
		SWEET_RICE_DRINK = "이 씨앗은 먹으러면 뭐가 이렇게 시간을 쓰는지..", -- 비타협 식혜
		PINEANANAS_JUICE = "과피가 없어도 많이 먹음 입에서 피나지.", -- 레기온 파인애플 주스
		...
		FRUITSODA = "탄산음료 좀 천천히 마신다고 오래 걸리지 않겠지.", -- 과일소다
		LEMONLIMESODA = "탄산음료는 달기떄문에 맛있지.", -- 레몬 라임소다(과일소다 상위 호환)
		COLA = "사레들리기 쉽다는 점만 빼면 훌룡해.", -- 콜라
		COLAQUANTUM = "흥미로워, 본래 이 시간선에서는 있을수 없는 음료야.", --콜라 퀀텀
		LUMPY_BEER = "찰나무 뿌리를 못 쓰게 된게 언제더라?", --루트비어(미국 찰나무 = Sassafras 굶지마 세계관 기준으로 미래인 1979년에 금지됨;)

		-- 양조주

		CORN_BEER = "술 마시고 즐길 시간이 어디 있어?", -- 콘 맥주
		MADHU = "단맛을 잃었어, 하지만 시간을 얻었지.", --벌꿀주
		WINE = "그렇게 빨리 식초가 되진 않을거야.", --와인 (그냥 베리 양조)
		NOBLEWINE = "그냥 투명한 와인 아닌가?", --노블 와인(구운 베리 양조)
		SPARKLINGWINE = "어느 시간선에서는 파티에서 샴페인을 터트리는 내가 있을수도 있겠지.", --스파클링 와인
		GLOWBERRYWINE = "이거지! 잘 상하지도 않는데 효율성도 갖췄어!", --글로우 베리 와인
		KUMIS = "시간을 투자한 가치가 있길 바래.", -- 크므즈,산양유주
		LUMPY_WINE = "이득 봤다고 봐야지.", --서주(감자 고구마등 뿌리 술)
		PULQUE = "시간 쓴 거에 비해 금방 상할것 같은데.", --풀케(선인주)
		RUINCACAO_WINE = "이상한 발상같긴 한데, 어찌되었든 시간은 벌었어.", -- 카카오 와인 

		-- 콜라보 소다

		CHERRY_DOCTOR_COLA = "이 음료가 시간여행을 하진 않겠지.", --체리숲 닥터 페퍼(닥터 체리) (대충 닥터후 패러디)
		PINEANANASSODA = "좋아, 즐기는데 오래 걸리지 않을거야.", -- 탄산 파인애플(오란씨)
		ORANGESODA = "좋아, 즐기는데 오래 걸리지 않을거야.", -- 탄산 오랜지(환타)
		STRAWBERRYSODA = "좋아, 즐기는데 오래 걸리지 않을거야.", -- 탄산 딸기(웰치스 딸기)

		--콜라보 양조주

		AUROBOW = "이게 가능한지나 모르겠지만 어찌되었든 훌룡해.", -- 난파선 오로보우,무지개 해파리주
		COCONUT_WINE = "만약에 항해가 길어진다면 챙겨야 겠지.", -- 난파선 코코넛 와인
		CHERRY_BLOOM_MADHU = "고급스럽네!", -- 체리숲 벚꽃꿀주
		GIANT_BLUEBERRY_WINE = "살짝 불안한걸.", -- 비타협 붐!베리 사파이어(불 붙히면 폭발함)
		RICE_WINE = "어차피 시간 들이지 않음 먹지도 못했어.", -- 비타협 막걸리
		WHEAT_BEER = "맥주로 만드는 방법도 나쁘지 않네.", -- 힙 오브 푸드 밀맥주
		PALE_BEER = "원채 이상한걸 많이 봐서 이정도는 놀랍지가 않아.", -- 레기온 페일에일 (몬스트라 열매를 쓴 청록색 맥주)

		--증류주

		DISINFECTANT = "나한텐 소용없어. 오직 시간만 필요해!", -- 소독약
		CORN_WHISKEY = "솔직히 술맛은 몰라, 거의 안 썩는다는 것만 알지.", -- 콘 위스키
		LUMPY_VODKA = "취해서 시간낭비 하는건 조심해야 해.", -- 보드카
		TEQUILA = "망설이지 말고, 눈 딱 감고 마셔!", -- 데킬라
		MADHU_RUM = "이걸들고 항해를 나갈수 있겠지, 내일 아니면 지금.", -- 럼주
		...
		ABSINTHE = "천천히 홀짝홀짝 마실 시간은 없어.", -- 압생트
		TEA_LUMPY_VODKA = "솔직히 차 향이 나는지도 모르겠어.", -- 녹차 리큐르
		PETALS_BERRY_BRANDY = "술에 꽃 향기 난다고 맡고 있을 여유 없어.", -- 장미향 브랜디
		KAHLUA = "환상적이야, 공포도 이겨내고 기운도 돋아.", -- 커피 리큐르 깔루아
		NUT_CORN_WHISKEY = "이게 무슨 술이고 뭐고 구분하기에는 시간이 아까워.", -- 너트 위스키
        RUINCACAO_CREAM_LIQUEUR = "입맛엔 맞지만, 과음은 조심해야 해.", --베일리스 카카오 크림 리큐르
        RUMCHATA = "거부감이 좀 줄어서 낫네.", -- 럼차타(럼에 오우차타 라는 유제품 섞은)

		--콜라보 증류주

		MOONWAY = "7빛깔로 빛나는 고도주로군.", -- 무지개 해파리주 증류주
		ARRACK = "섬 이곳 저곳을 돌아다녀도 시간으로부터 자유로워.", -- 아락 인도네시아 버전 코코넛 와인 증류주
		CHERRY_BLOOM_RUM = "예쁜 술병이지만 구경한다고 시간을 허비하진 않을거야.", -- 체리 꿀주 증류주 벛꽃잎 떠다
		RICE_SPIRITS = "한잔이면 바싹 정신 차릴수 있을거야.", --청주
		WHEAT_WHISKEY = "솔직히 술맛은 몰라, 거의 안 썩는다는 것만 알지.", -- 밀 위스키
		PALE_WHISKEY = "입 안에 들어가면 다 똑같은 술이야.", -- 페일 몰트 위스키

		--왈리 전용 콜라보 증류주

		NUT_COCONUT_BRANDY = "바다는 변덕스럽지, 공포를 이겨낼 것이 필요해.", -- 코코넛 브랜디 (캐러비안 럼을 만들수 없어서 카리브 브랜디 라는 이름으로 대체)
        ...
        RUINCOLATE = "달콤한데다 시간을 잘 타지도 않지!", -- 그냥 초콜릿
        RUIN_SCHOKAKOLA = "완벽한 간식이야!", -- 쇼카콜라

		BUCKET_STEEL_EMPTY = 
		{
			EMPTY = "굳이 이걸 멀뚱멀뚱 쳐다 봐야 할 시간이 있나?", --빈 양철 양동이
		...
		BUCKET_WOODIE_EMPTY = 
		{
			EMPTY = "우디의 나무 다루는 솜씨는 존중할만해, 벌써 속을 다 비워 냈잖아.", --우디의 빈 나무 양동이
			FULL = "시상이 변하기전에 얼른 회수하자!", -- 깨끗한 빗물이 참
			FULL_ICE = "아이고, 시간째로 굳어 버렸어.", --빗물이 얼어버림
			DIRTY = "아이고야, 시간에 오염되어 버렸네", --빗물이 더러워짐
			DIRTY_ICE = "아이고야, 오염된채로 굳어버렸네.", --더러운 빗물 얼어버림
		...
		DESALINATOR =
		{
			...
			PURIFY_LONG = "끝날때까지 가만히 서서 기다릴수 없어, 시간이 부족해!", -- 정수 끝나기까지 많이 남음
			PURIFY_SHORT = "으으 빨리 빨리!", -- 정수가 곧 끝남
			HASWATER = "드디어!", --제염기에 깨끗한 물이 있음
			BURNT = "차질이 좀 생겼네.",   --제염기가 타버림
			HASSALT = "내 정신좀 봐, 소금부터 제거해야 해!", --제염기에 소금이 남아 있음
		},

		BARREL =
		{
			GENERIC = "적어도 물이 썩을 걱정은 치워 놔도 되겠어.", -- 물통 조사 대사
			BURNT = "누가 물 담은 통이 불탈거라 예측할수 있겠어?", -- 물통이 타버림
		},

		BREWERY =
		{
			EMPTY = "시간 먹는 하마가 따로 없네.", --양조기 조사 대사
			BURNT = "그렇게 될 운명 이었나보지.", --양조기 타버림
			FERMENTING_LONG = "발효란건 시간이 걸려, 멍하니 기다릴수 없다는 예기야.", --양조 끝나기 까지 많이 남음
			FERMENTING_SHORT = "참 오래도 기다렸네!", --양조가 곧 끝남
			DONE = "이제야 뭐가 나왔네!", --양조기에 음료가 들어있음
		},

		CAMPKETTLE_ITEM = "한동안 가지고 다녀야겠어.", --휴대 주전자 아이템 조사 대사
		CAMPKETTLE =
		{
			GENERIC = "내가 물 넣는걸 잊었나?", -- 화덕에 건 휴대 주전자 안에 물이 없음
			BOILING_LONG = "으으 빨리 좀 서둘렀으면!", -- 물 끓이기 시작함
			BOILING_SHORT = "얼마나 뜸 들어야 하는거지?", -- 물 거의 다 끓음
			DONE = "깨끗한 물 하나 얻는데 이렇게 시간이 걸린다니..", --깨끗한 물이 들음
			STOP = "아이고야, 화력이 부족해!", --화덕에 불이 없어서 더러운물이 끓지 않는 상태
		},

		CAMPDESALINATOR_ITEM = "어딜 가도 바다가 있으니깐 쓸모가 있을거야.", --휴대 제염기 아이템 조사 대사
		CAMPDESALINATOR = 
		{
			GENERIC = "여기에 물이 차 있던게 언제였지, 처음부터 없었나?", --물 없음
			BOILING_LONG = "뭐가 이렇게 오래 걸리는건지..!", --물 끓이기 시작
		...
		KETTLE =
		{
			EMPTY = "적어도 한번에 물을 많이 끓일수 있을 것 같네.", --빈 주전자 조사대사
			BURNT = "아이고, 한동안 이슬이나 핥아야 겠네!", --주전자가 불탐
			...
			PURIFY_LONG = "으으 빨리 좀 서둘렀으면!", -- 더러운 물을 끓이기 시작한지 얼마 안됨
		...
		DISTILLERS =
		{   
		    EMPTY = "이걸로 만든 술은 적어도 쉽게 썩진 않겠지.", --증류기 조사 대사
			DISTILLTING_LONG = "이게 끝나기까지 지켜보고 있다간 내가 먼저 늙어 죽겠어.", --증류가 끝나기 까지 많이 남음
		...
		WELL_WATERPUMP = {
			...
			LOW_PRESSURE = "아무래도 힘이 덜한것 같아.", -- 압력이 거의 없을때
