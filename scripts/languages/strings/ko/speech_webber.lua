return {
	...
	DESCRIBE =
	{
		...
		TEALEAVES = "좀 독특한 향이 나는 잎이야.", --찻잎
		TEALEAVES_DRIED = "뭔가 좀 구수해.", --말린 찻잎

		CAFFEINBERRY = "콩 들이 달려있는 것 같아.", --커피 덤불
		DUG_CAFFEINBERRY = "조금만 사랑을 가져주면 다시 자랄거야.", --파낸 커피덤불
		CAFFEINBERRY_BEAN = "이걸로 칠리 콘 카르네를 만드는 걸까?", --생 커피콩
		CAFFEINBERRY_BEAN_COOKED = "씹어봤는데 엄청 써.", --구운 커피콩

		RUINCACAO_TREE = "또 다른 열대 동굴 나무 겠지?", --카카오 나무
		DUG_RUINCACAO_TREE = "땅으로 보내줄게!", --파낸 카카오 나무
		RUINCACAO_SEED = "우와아, 꽃이 폈어!", --카카오 새싹 아이템
		RUINCACAO_SEED_SAPLING = "쑥쑥 커서 멋진 열매를 맺어 주럼.", -- 카카오 새싹 심은거
		...
		FRUITJUICE = "여러 과일을 담은 주스야.", --잡 과일쥬스
        ...
        TOFFEE_LATTE = "녹인 스카치 캔디 같아!", --토피넛 라떼
		RUINCHA_LATTE = "맛있다고 너무 많이 마시지 말래.", --모카 라떼(커피+초코)
		...
		BUTTERBEER = "웬디가 유명한 소설에 나오는 맛있는 음료라고 알려줬어!", --해리포터 시리즈의 버터비어
		...
		PINEANANAS_JUICE = "마시고 나면 이가 좀 뻑뻑해.", -- 레기온 파인애플 주스
		ORANGE_JUICE = "양치는 좀 미뤄야 겠네.", -- 모어 푸드 팩 오랜지 주스
		...
		MADHU = "으으음, 전혀 달지않고 그냥 술이야.", --벌꿀주
		WINE = "떫어!", --와인 (그냥 베리 양조)
		NOBLEWINE = "아마 달것 같은데, 조금만 마셔볼까?", --노블 와인(구운 베리 양조)
		SPARKLINGWINE = "흔들어 보고 싶지만 하면 안되겠지?", --스파클링 와인
		...
		PINEANANASSODA = "이가 안 썩게 조심해야지.", -- 탄산 파인애플(오란씨)
		...
		NUT_CORN_WHISKEY = "우디 아저씨가 좋아 할것 같아!", -- 너트 위스키
        RUINCACAO_CREAM_LIQUEUR = "와! 초콜릿 술이잖아?", --베일리스 카카오 크림 리큐르
        RUMCHATA = "으으음, 그냥 우유를 줘.", -- 럼차타(럼에 오우차타 라는 유제품 섞은)

		--콜라보 증류주

		MOONWAY = "색이 예뻐!", -- 무지개 해파리주 증류주
		ARRACK = "우린 그냥 야자 열매나 먹어야 겠어.", -- 아락 인도네시아 버전 코코넛 와인 증류주
		CHERRY_BLOOM_RUM = "으음, 거미 친구들 집에 장식해 볼까?", -- 체리 꿀주 증류주 벛꽃잎 떠다님
		...
		BARREL =
		{
			GENERIC = "우리 물을 저장 해 둘수 있지.", -- 물통 조사 대사
			BURNT = "이제는 우리한 별로 쓸모가 없을거야.", -- 물통이 타버림
		},

		BREWERY =
		{
			EMPTY = "이 통 안으로 숨는 건 좋은 생각이 아닌 것 같아!", --양조기 조사 대사
			BURNT = "으음, 다 타버렸네.", --양조기 타버림
			FERMENTING_LONG = "선생님이 이런 일은 오래 기다려야 한다고 했어.", --양조 끝나기 까지 많이 남음
			FERMENTING_SHORT = "곧 끝날것 같아!", --양조가 곧 끝남
			DONE = "음료가 준비 된 것 같아!", --양조기에 음료가 들어있음
		},

		CAMPKETTLE_ITEM = "내 팔 하나로 들고 다녀야 겠어!", --휴대 주전자 아이템 조사 대사
		CAMPKETTLE =
		{
		...
		KETTLE =
		{
			...
			PURIFY_LONG = "연못 물 그대로로는 음료는 못 만든데.", -- 더러운 물을 끓이기 시작한지 얼마 안됨
		...
		DISTILLERS =
		{   
		    EMPTY = "우와, 엄청 큰 과학 실험 장치 같아.", --증류기 조사 대사
		...
		WELL_WATERPUMP = {
			...
			RECHARG_PRESSURE = "으음, 내가 물을 길어 올게!", -- 압력이 아예 없을때(여기까지 쓰면 압력이 최대가 될때까지 못씀.)
		},
