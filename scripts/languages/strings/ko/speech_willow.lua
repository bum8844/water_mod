return {
	ACTIONFAIL =
	{
		GIVE =
		{
			WELL_NOTEMPTY = "일단 있는 것 부터 치워야지.", -- 우물에 말라 빈 양동이 달려있는데 양동이 달려 함
			WELL_BUSY = "우물 쭈물 하고 있는게 맘에 안들어!", -- 우물에 물 든 양동이 달려있는데 달려 함
		}
	},
	...
	ANNOUNCE_NOTCHARGE = "어머, 너 젖이 안 나오는구나.", --번개염소가 충전이 안되서 양동이로 젖을 짤수 없음
	...
	DESCRIBE =
	{
		...
		WELL_WATERPUMP_KIT = "나 기계 조립 같은거 잘 못한단 말이야.", --스팀 물펌프 키트
		WELL_BURYING_KIT = "잘 됬네. 마침 물 소리가 거슬렀어.", -- 구멍 매설 키트
		WELL_DRILLING = "불을 붙히면 좀 조용해 질까?", --드릴로 구멍 뚫는 중
		...
		TEA_SEED_SAPLING = "곧 진짜 나무가 될 수 있을거야.", --차나무 새싹
		TEALEAVES = "태우면 좋은 향이 날 것 같네.", --찻잎
		TEALEAVES_DRIED = "땔감으로 쓰기 딱 좋아 보이네.", --말린 찻잎

		CAFFEINBERRY = "이번엔 차가운 달에서 널 보네.", --커피 덤불
		DUG_CAFFEINBERRY = "이제 내가 널 어떻게 해 줄까?", --파낸 커피덤불
		...
		ADDITIVE_NUT = "태우면 더 고소할 것 들이야.", -- 리큐르에 제조에 첨가 할 버치넛,코코넛,커피콩,카카오콩 첨가제
		ADDITIVE_DAIRY = "술에 우유는 왜?", -- 리큐르에 제조에 첨가할 유제품 첨가제

		-- 음료

		WATER_CLEAN = "내가 다 마셔서 없에버려야 겠어.", --깨끗한 물
		WATER_DIRTY = "응, 아니야.", --더러운 물
		WATER_SALTY = "세상엔 물이 왜 이리 많을까?", --바닷물

		GOOPYDRINK = "차라리 불 속에 넣었으면 이렇게 될 일은 없었잖아.", --실패한 음료(웻 굽 개념)
		...
		DRAGONJUICE = "불 같은 과일을 왜 물에 넣었어?", --용과 쥬스
		...
		LOTUSTEA = "물 속 으로 돌아갔으니깐 만족 했을거야.", --연꽃차
		SUSHIBISCUS = "누구 식탁 위에 올려볼까?", -- 자살차
		...
		TEA_RUINCOLATE_LATTE = "왜 핫 초코에 잎을 갈아 넣은거야?", -- 말차 라떼

		MULLED = "아, 뱃속에 난로 켜 둔것 같아.", --뱅쇼
		...
		PINEANANAS_JUICE = "마실떈 좋은데, 마시고 나면 입안이 얼얼해 질거야.", -- 레기온 파인애플 주스
		...
		MADHU = "꿀로 만들었는데 달진 않네.", --벌꿀주
		WINE = "오크 통 째로 태우면 향이 엄청날 텐데.", --와인 (그냥 베리 양조)
		NOBLEWINE = "가볍게 잘 넘어 가네.", --노블 와인(구운 베리 양조)
		...
		RUINCACAO_WINE = "뜨거운 핫 초코가 더 좋기는 하는데 이것도 나름 괜찮네.", -- 카카오 와인 
		...
		AUROBOW = "해파릴 태워도 이렇게 색이 바껴가면서 타진 않는데 말이지.", -- 난파선 오로보우,무지개 해파리주
		COCONUT_WINE = "술은 맛있네, 나무는 태워버려.", -- 난파선 코코넛 와인
		CHERRY_BLOOM_MADHU = "난 불타서 흩날리는 꽃잎이 더 재밌어.", -- 체리숲 벚꽃꿀주
		GIANT_BLUEBERRY_WINE = "내 맘에 드는 성질로 바뀌었어!", -- 비타협 붐베리 와인(불 붙히면 폭발함)
		RICE_WINE = "이상한걸로 술도 만드네.", -- 비타협 막걸리
		WHEAT_BEER = "입가에 거품 묻히긴 싫어.", -- 힙 오브 푸드 밀맥주
		PALE_BEER = "어째서 맥주가 푸른 색이야?", -- 레기온 페일에일 (몬스트라 열매를 쓴 청록색 맥주)
		...
		BERRY_GIN = "내가 진정으로 태우고 싶었던 건 이 썩어빠진 세상이였다구.", --진
		GLOWBERRY_GIN = "빛도 나고, 내 목에 불꽃을 붙혀줄거야.", --글로우 베리 진
		ABSINTHE = "방화도 예술로 봐 달라구우.", -- 압생트
		...
		ARRACK = "은은하게 열매 향이 나는게 맘에 들어.", -- 아락 인도네시아 버전 코코넛 와인 증류주
		CHERRY_BLOOM_RUM = "예쁘네, 다 마시고 불 좀 질러봐야 겠어.", -- 체리 꿀주 증류주 벛꽃잎 떠다님
		...
		BUCKET_WOODIE_EMPTY = 
		{
			EMPTY = "속을 비워낸 땔감 아니야?", --우디의 빈 나무 양동이
			FULL = "빗물이 고여있어.",  --깨끗한 빗물이 참
			FULL_ICE = "통째로 얼어버렸네.", --빗물이 얼어버림
			DIRTY = "더러운 물이 고여있잖아!", --빗물이 더러워짐
		...
		DESALINATOR =
		{
			...
			PURIFY_LONG = "불을 안 쓰니깐 지루하네.", -- 정수 끝나기까지 많이 남음
			PURIFY_SHORT = "조만간 정수가 끝날거야.", -- 정수가 곧 끝남
			HASWATER = "정수가 끝났어.", --제염기에 깨끗한 물이 있음
			BURNT = "적어도 엄청나게 재미 있었잖아?",   --제염기가 타버림
			HASSALT = "소금을 먼저 치워야 쓸 수 있어.", --제염기에 소금이 남아 있음
		},

		BARREL =
		{
			GENERIC = "물 통 말고 화약통은 어때?", -- 물통 조사 대사
		...
		KETTLE =
		{
			EMPTY = "모든 물은 불로 살균 해야 해.", --빈 주전자 조사대사
		...
		{   
			...
			DISTILLTING_LONG = "안쪽에 불을 던저 넣었으면 더 빨리 끝나지 않았을까?", --증류가 끝나기 까지 많이 남음
		...
		WELL_SPRINKLER =
		{ 
			LOWFUEL = "내가 쟤 한데 연료를 공급 해 줘야 해?", -- 연료가 거의 없는 상태에서의 조사 대사
			ON = "정말 신경을 긁는 소릴 내네.", -- 작동중임
			OFF = "내 방해는 하지 마렴.", --작동을 멈춘상태에서의 조사 대사
		},	
	},
}