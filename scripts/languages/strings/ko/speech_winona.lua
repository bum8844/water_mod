return {
	ACTIONFAIL =
	{
		GIVE =
		{
			WELL_NOTEMPTY = "그렇게 할거였으면 설계때부터 도르래를 하나더 달았어야지.", -- 우물에 말라 빈 양동이 달려있는데 양동이 달려 함
	...
	ANNOUNCE_DCAPACITY_HALF = "음, 이제좀 절제하는게 좋겠어.", -- 취기가 돌기 시작

	DESCRIBE =
	{
		BOILBOOK = "음료와 술을만드는 방법이 잔뜩 적혀있군.",
		...
		WELL_BURYING_KIT = "땅에 뚫린 구멍을 위험하게 방치해두면 안되지, 암.", -- 구멍 매설 키트
		WELL_DRILLING = "튀어오는 돌에 안 맞게 조심해야지.", --드릴로 구멍 뚫는 중
		WELL_DRILLING_ITEM = "작동시키기전에 땅에 단단히고정시키고, 안전수칙 지켜!", --드릴 아이템
		...
		TEA_SEED_COOKED = "먹을수 있겠는데, 알 방법은 별 거 없지!", --구운 차나무 열매
		TEA_SEED_SAPLING = "이 나무는 아직 만들어지는 중이야.", --차나무 새싹
		TEALEAVES = "이파리 주제에 향기로운데?", --찻잎
		TEALEAVES_DRIED = "뜨거운 물좀 준비 해 볼까.", --말린 찻잎

		CAFFEINBERRY = "커피콩이 열리잖아! 세상에.", --커피 덤불
		DUG_CAFFEINBERRY = "이걸 다시 땅에 심어야겠어, 내 땅이 있다면 말이야, 하!", --파낸 커피덤불
		CAFFEINBERRY_BEAN = "먼저 불속에 던져 넣어야 겠지?", --생 커피콩
		CAFFEINBERRY_BEAN_COOKED = "아, 아침에 맡는 커피 원두 향기란!", --구운 커피콩

		RUINCACAO_TREE = "어, 음, 가지를 뻗는 청사진부터 잘못되었나 보네.", --카카오 나무
		DUG_RUINCACAO_TREE = "다시볼수록 이상하게 생긴 나무야.", --파낸 카카오 나무
		...
		RUINCACAO_BEAN = "먹을수 있는 기름덩어리로는 안 보이는군.", --부순 후 카카오 콩  
		...
		ADDITIVE_NUT = "입이 심심할때 먹으라고 모아둔건 아니지?", -- 리큐르에 제조에 첨가 할 버치넛,코코넛,커피콩,카카오콩 첨가제
		ADDITIVE_DAIRY = "우유 크림이네.", -- 리큐르에 제조에 첨가할 유제품 첨가제

		-- 음료

		WATER_CLEAN = "깨끗한 물 한잔에도 고마워해야 한다니..", --깨끗한 물
		WATER_DIRTY = "아무리 급해도 고인물을 마시면 안되지.", --더러운 물
		...
		BLACKTEA_ICED = "땀 좀 흘리고 나서 마셔야겠다.", -- 홍차 아이스티
		FUER = "의외로 향이 나쁘지 않네.", --보이차,이파리차,고사리차
		MIXFLOWER = "고상한 귀부인취향인것 같은데 향이 좋긴좋네.", --잡 꽃잎 차
		HIBISCUSTEA = "과일하나 안보이는데 시큼달달하네.", --히비스커스 차
		CACTUSFLOWER_TEA = "꽃으로 차 하는게 유행인갑네.", --선인꽃차
		LOTUSTEA = "꽤 이국적인걸.", --연꽃차
		SUSHIBISCUS = "뭐 이런 차가 다 있지.", -- 자살차
		MOON_BLOSSOM_TEA = "후! 정신이 반짝 드네 그려.", --달꽃차

		CAFFEINBERRY_JUICE = "야근 뛸때 자주 달고 살았지.", -- 블랙 커피
        TOFFEE_LATTE = "난 커피에 설탕 안 넣는 파인데.", --토피넛 라떼
		RUINCHA_LATTE = "커피와 초코, 둘중에 하날 선택하기 어렵다면야.", --모카 라떼(커피+초코)
		
		HOTRUINCOLATE = "포근한걸.", --핫초코
		RUINCOLATE_SMOOTHIE = "시판 음료로 본적있기는 하는데, 사서 마실일은 없었지.", --초코 스무디(차가움)
		RUINXOCOLATL = "이거 배기가스만큼이나 향미가 독한데.", --쇼콜라틀(매캐함)
		TEA_RUINCOLATE_LATTE = "혼란스럽네, 이건 밀크티야 초코스무디야?", -- 말차 초코 라떼

		MULLED = "내가 한방이 있는 여자는 아니지만.. 한 잔은 좋아한다고! 헤헤!", --뱅쇼
		...
		QUARTZ_SHAKE = "이정도면 양호하지.", --난파선 광대 물고기 쉐이크
		CHERRY_ROSEBERRIES_TEA = "신맛이 또 별미로군.", --체리숲 로즈힙 차
		CHERRY_HONEY_TEA = "한잔해서 목에 걸린 먼지를 걸러내야겠어.", --체리숲 벚꽃꿀 차
		SWEET_RICE_DRINK = "흠, 꽤 달군.", -- 비타협 식혜
		PINEANANAS_JUICE = "통조림에 든 국물보다는 확실히 낫네.", -- 레기온 파인애플 주스
		ORANGE_JUICE = "탕비실 냉장고에 이게 없는날을 본적이 없어.", -- 모어 푸드 팩 오랜지 주스
		STRAWBERRY_SMOOTHIE = "여공들 취향 같은데, 아,나도 여공이지. 흠!", -- 모어 푸드 팩 딸기 스무디
		STRAWBERRY_MILK_SMOOTHIE = "찻집 디저트음료 같은데 맛있네.", -- 모어 푸드 팩 딸기 스무디 라떼
		...
		SODA = "플레인 탄산수로군.", -- 탄산수
		FRUITSODA = "파티갈 일도 없으니 이거 먹을일도 없었지.", -- 과일소다
		LEMONLIMESODA = "이빨 썩겠는데.", -- 레몬 라임소다(과일소다 상위 호환)
		COLA = "꼭 마시러 하기만하면 이미 누가 다 마셨더라.", -- 콜라
		COLAQUANTUM = "워, 이거 마셔도 안전한거야?", --콜라 퀀텀
		LUMPY_BEER = "콜라보다 맛있는데 아무도 내편 안 들어준단말이지.", --루트비어(술이 아님)

		-- 양조주

		CORN_BEER = "일과 마치고 한잔하면 딱이야!", -- 콘 맥주
		MADHU = "위그가 좋아할것 같네.", --벌꿀주
		WINE = "향을 음미할 시간은없어, 해야할 일이 산더미인걸.", --와인 (그냥 베리 양조)
		NOBLEWINE = "이런 고상한건 나랑 거리가 있단 말이지.", --노블 와인(구운 베리 양조)
		SPARKLINGWINE = "맥스 면전에서 흔들어줘야지.", --스파클링 와인
		GLOWBERRYWINE = "수제 양조의 로망이로군.", --글로우 베리 와인
		KUMIS = "우유로 술을 만드는건 처음보네.", -- 크므즈,산양유주
		LUMPY_WINE = "꽤 시원한맛이 나는데?", --서주(감자 고구마등 뿌리 술)
		PULQUE = "선인장으로 술도 만들어지는군.", --풀케(선인주)
		RUINCACAO_WINE = "카카오로 와인이 만들어졌다면, 만들수있는거겠지.", -- 카카오 와인 

		-- 콜라보 소다

		CHERRY_DOCTOR_COLA = "이건 콜라가 아닌데.", --체리숲 닥터 페퍼(닥터 체리)
		PINEANANASSODA = "시판 탄산음료 파인애플맛이네.", -- 탄산 파인애플(오란씨)
		ORANGESODA = "시판 탄산음료 오렌지맛이야.", -- 탄산 오랜지(환타)
		STRAWBERRYSODA = "딸기맛 탄산음료가 있다해도 놀랍진않지.", -- 탄산 딸기(웰치스 딸기)

		--콜라보 양조주

		AUROBOW = "무지개 술이네, 원재료는 좀 거시기하지만.", -- 난파선 오로보우,무지개 해파리주
		...
		GIANT_BLUEBERRY_WINE = "블루베리로 와인을 못만들것도 없지.", -- 비타협 붐베리 와인 (불 붙히면 폭발함)
		RICE_WINE = "술이 좀 탁하네.", -- 비타협 막걸리
		WHEAT_BEER = "작업끝나면 한잔 걸쳐야지!", -- 힙 오브 푸드 밀맥주
		PALE_BEER = "맥주 색이 좀 이상하면 어떠하리.", -- 레기온 페일에일 (몬스트라 열매를 쓴 청록색 맥주)

		--증류주

		DISINFECTANT = "응급처치용이지.", -- 소독약
		CORN_WHISKEY = "난 찔금찔금 마시는것보다 왕창 마시는게 취향인데.", -- 콘 위스키
		LUMPY_VODKA = "날씨가 추워질떄까지 가지고있어야겠다.", -- 보드카
		TEQUILA = "향이 꽤 터프한데!", -- 데킬라
		MADHU_RUM = "뱃사람이 물에 럼을섞어서 식수를 보존하는시대는 좀 됬지.", -- 럼주
		BERRY_BRANDY = "이런 비싼술을 마셔볼기회가 별로없는데 말이지.", -- 베리 브랜디
		AREUHI = "본적없는 양주네.", -- 크므즈 증류주 아르히

		--왈리 전용 증류주

		BERRY_GIN = "이거 때문에 나락간 친구를 몇명 알지.", --진
		...
		KAHLUA = "아무래도 술과 커피는 따로 마셔야겠다.", -- 커피 리큐르 깔루아
		...
		MOONWAY = "열량 높아 보이는데 연료로도 쓸수 있을까?", -- 무지개 해파리주 증류주
		ARRACK = "야자 단내가 나는군.", -- 아락 인도네시아 버전 코코넛 와인 증류주
		CHERRY_BLOOM_RUM = "공항 기념품 같아보이는데, 어차피 다 같은 술이야.", -- 체리 꿀주 증류주 벛꽃잎 떠다
		...
		NUT_COCONUT_BRANDY = "꽤 달달한 향이나네.", -- 코코넛 브랜디 (캐러비안 럼을 만들수 없어서 카리브 브랜디 라는 이름으로 대체)
	    GIANT_BLUEBERRY_GIN = "식용보다는 공업용에 가깝지않나?", -- 붐 베리 사파이어 (당근 폭발함)

		--냄비 요리

        DARK_RUINCOLATE = "친구가 쉬엄쉬엄하라고 간식을 준비했나보네.", --다크 초콜릿.
        WHITE_RUINCOLATE = "오 왈리, 나 단거 싫어하는거 알잖아.", --화이트 초콜릿
        RUINCOLATE = "녹기 쉬우니깐 잘 먹진 않았지.", -- 그냥 초콜릿
        RUIN_SCHOKAKOLA = "이거야, 잘 녹지도 않고, 케이스도 있고 일하면서 힘도 나지.", -- 쇼카콜라
		...
		BUCKET_STEEL_EMPTY = 
		{
			...
			DIRTY_ICE = "한겨울에 청소하고 물 버리는걸 잊었나보군.", --더러운 빗물이 얼어버림
		},
		...
		BUCKET_WOODIE_EMPTY = 
		{
			...
			FULL_ICE = "빼려면 고생해야 겠는데?", --빗물이 얼어버림
			DIRTY = "걸레라도 한번 빤 물 같군.", --빗물이 더러워짐
			DIRTY_ICE = "한겨울에 청소하고 물 버리는걸 잊었나보군.", --더러운 빗물 얼어버림
		},

		BUCKET_DRIFTWOOD_EMPTY = 
		{
			EMPTY = "워, 이거 위생안전기준 인증은 못 받겠는데?", -- 유목양동이
			FULL = "빗물이 고였네.", -- 깨끗한 빗물이 참
			FULL_ICE = "빼려면 고생해야 겠는데?", --빗물이 얼어버림
			DIRTY = "걸레라도 한번 빤 물 같군.", --빗물이 더러워짐
			DIRTY_ICE = "한겨울에 청소하고 물 버리는걸 잊었나보군.", --더러운 빗물 얼어버림
		...
		DESALINATOR =
		{
			...
			HASWATER = "안에든게 바닷물이 아님을 보증하지.", --제염기에 깨끗한 물이 있음
			BURNT = "이건 좀 열받네.",   --제염기가 타버림
			HASSALT = "필터박스에서 소금부터 제거해야해.", --제염기에 소금이 남아 있음
		},

		BARREL =
		{
			GENERIC = "나랑 우디가 물이 썩지않을 방법을 고민했지.", -- 물통 조사 대사
		...
		BREWERY =
		{
			...
			BURNT = "아무리그래도 이건 화내야 겠는걸.", --양조기 타버림
			FERMENTING_LONG = "발효라는게 원래 오래 걸린다고 했지?", --양조 끝나기 까지 많이 남음
			FERMENTING_SHORT = "거의 다 되지 않았을까?", --양조가 곧 끝남
			DONE = "어디한번 맛 좀 볼까?", --양조기에 음료가 들어있음
		},

		CAMPKETTLE_ITEM = "공장제보단 좀 무거울거야.", --휴대 주전자 아이템 조사 대사
		CAMPKETTLE =
		{
			GENERIC = "내가 물을 가져와야겠군.", -- 화덕에 건 휴대 주전자 안에 물이 없음
			BOILING_LONG = "아직 68°F 정도로군.", -- 물 끓이기 시작함
			BOILING_SHORT = "192.2°F로 끓고 있어.", -- 물 거의 다 끓음
			DONE = "충분해 보이는데, 확인해 봐야겠지.", --깨끗한 물이 들음
			STOP = "워, 아직 마시면 안될거야.", --화덕에 불이 없어서 더러운물이 끓지 않는 상태
		},

		CAMPDESALINATOR_ITEM = "'휴대용'이라기에는 많이 무겁긴하지.", --휴대 제염기 아이템 조사 대사
		CAMPDESALINATOR = 
		{
			GENERIC = "텅 빈 깡통이네.", --물 없음
			BOILING_LONG = "아직 바닷물쪽의 수위가높아.", --물 끓이기 시작
			BOILING_SHORT = "이제 담수쪽 수위가 더 높군.", --거의 다 끓음 
			DONE = "담수가 다모였군!", -- 깨끗한 물이 들음
		...
		PORTABLEKETTLE_ITEM =
		{
			GENERIC = "품질면에서 공장제 티포트에 뒤쳐지지않아.", -- 휴대용 주전자(아이템) 조사 대사
			EMPTY = "물 한 방울 없군.", -- 설치한 빈 휴대용 주전자 조사 대사
			BOILING_LONG = "좋은건 다 시간이 필요한 법이야.", -- 음료를 만들기 시작한지 얼마 안됨
			BOILING_SHORT = "눈 깜짝할새면 다 되겠어!", --음료 조리가 거의 끝나감
		...
		{   
			...
			DISTILLING_SHORT = "끝나기까지 얼마남지 않았어.", --증류가 곧 끝남
			DONE = "어디 한번 맛 좀 볼까?", --증류기에 음료가 들어있음
		},

		WELL_WATERPUMP = {
			HIGH_PRESSURE = "약 123 psi의 압력으로 물을 끌어오고있어.", -- 압력 최대
			MIDDLE_PRESSURE = "약 81 psi의 압력으로 물을 끌어오고있어.", -- 압력이 중간
			LOW_PRESSURE = "약 47 psi의 압력으로 물을 끌어오고있어.", -- 압력이 거의 없을때
			RECHARG_PRESSURE = "이 압력으로는 물을 끌어올릴때까지 엄청 오래걸릴거야.", -- 압력이 아예 없을때(여기까지 쓰면 압력이 최대가 될때까지 못씀.)
		},
		
		WELL_SPRINKLER =
		{ 
			LOWFUEL = "연료를 채워줘야 해.", -- 연료가 거의 없는 상태에서의 조사 대사
			ON = "내 장비를 들고는 가까이가지 말아야지.", -- 작동중임
			OFF = "일정부분은 '마법'으로 작동해.", --작동을 멈춘상태에서의 조사 대사
		},	
	},
}