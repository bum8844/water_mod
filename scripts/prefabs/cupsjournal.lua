mods=rawget("mods")or(function()local m={}rawset("mods",m)return m end)()
...
local assets =
{
    Asset("ANIM", "anim/book_rec.zip"),
	Asset("IMAGE", "images/inventoryimages/cupsjournal.tex"),
	Asset("ATLAS", "images/inventoryimages/cupsjournal.xml")
}

local rec_phrases =
{
    "Tea: Fist add water, then add Tea brew!",
    "Coffee: Fist add water, then coffe beans!",
    "Dragon Drink: Fist add water, then Dragon Fruty!",
    "Monster Drink: Fist add water, then Monster brew!",
	"Ghosty Drink: Fist add water, then Ghosty brew!"
}

local function say(inst)
    inst.components.talker:Say(rec_phrases[math.random(#rec_phrases)])
end
...
local function fn()
    ...
    inst.AnimState:SetBuild("book_rec")
    ...
    inst.replica.inventoryitem:SetImage("cupsjournal")
    inst.components.inventoryitem.atlasname = "images/inventoryimages/cupsjournal.xml"
	
	inst:AddComponent("playerprox")
    inst.components.playerprox:SetDist(2.3, 2.3)
...
return Prefab("cupsjournal", fn, assets)
]]
local assets =
{
