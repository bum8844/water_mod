...
local function ChangeToItem(inst)
    if inst.components.brewing.product ~= nil and inst.components.brewing:IsDone() then
        inst.components.brewing:Harvest()
    end
    if inst.components.container ~= nil then
        inst.components.container:DropEverything()
    end
    local item = SpawnPrefab("distillers_item")
...
local function spoilfn(inst)
    if not inst:HasTag("burnt") then
        ...
        inst.AnimState:OverrideSymbol("swap_cooked", "cook_pot_food", inst.components.brewing.product)
...
local function getstatus(inst)
    return (inst:HasTag("burnt") and "BURNT")
        ...
        or (inst.components.brewing:GetTimeToCook() > 15 and "COOKING_LONG")
        or "COOKING_SHORT"
end
...
local function ondeploy(inst, pt, deployer)
    local spicer = SpawnPrefab("distillers")
    if spicer ~= nil then
...
return Prefab("distillers", fn, assets, prefabs),
    MakePlacer("distillers_item_placer", "portable_spicer", "portable_spicer", "idle_empty"),
    Prefab("distillers_item", itemfn, assets_item, prefabs_item)
