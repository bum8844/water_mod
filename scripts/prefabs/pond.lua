...
local function UpdateState(inst)
	inst.BreakTheIce.state = 'pending'
	inst.BreakTheIce.OnSnowLevel(inst)
end

local function OnFinish(inst)
	inst.BreakTheIce.state = 'broken'
	inst.frozen = false
	
	inst:RemoveComponent('workable')
	
	if inst.BreakTheIce.task then
		inst.BreakTheIce.task:Cancel()
		inst.BreakTheIce.task = nil
	end
	
	inst.BreakTheIce.targettime = GetTime() + 30--*(3 + 3*math.random())
	inst:DoTaskInTime(inst.BreakTheIce.targettime, UpdateState)
...
local function OnSnowLevel(inst, snowlevel)
	...
	local snowlevel = TheWorld.state.snowlevel or 0

    if snowlevel > .02 then
        if not inst.frozen and inst.BreakTheIce.state ~= 'broken' then
            inst.frozen = true
    ...
    elseif inst.frozen then
        ...
        inst.BreakTheIce.state = 'unfrozen'
        inst:RemoveComponent('workable')
